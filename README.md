...
6. 填写 `01_outline/volumes/volume_index.md` 与当前卷文件（卷首概览 + 卷内约束）；需要时新增 `volume_XX_stage_control.md`。
7. 为每章建立 `01_outline/scene_cards/chXX_scene_plan.md`（场景卡）。
8. 前 1-3 章完成后，更新 `04_chapter_memory/front_3_chapters_overview.md` 与章节摘要。
9. 每章写作前运行 `08_tools/build_context_pack.ps1` 生成最小上下文包。
10. 每章写完后更新 `rolling_memory.md`、`canon_facts.md`、`open_questions.md`、`timeline_ledger.md`、`character_state.md`、`faction_memory.md`。
11. 每 10 章产出一份 `arc_memory_arcXX.md`（500-800 字压缩记忆），后续优先喂这份摘要。
12. 每卷维护 `04_chapter_memory/volume_memory/volume_XX_memory.md`，卷末复盘并更新下一卷约束。
...
- `01_outline/volumes/volume_XX_*.md`（当前卷）
- `04_chapter_memory/volume_memory/volume_XX_memory.md`（当前卷记忆）
- `04_chapter_memory/faction_memory.md`（势力记忆）
...
- `01_outline/volumes/` 分卷索引、卷首概览、卷内执行约束
- `01_outline/volumes/volume_XX_stage_control.md` 阶段细化控制（可选但推荐）
- `02_characters/` 人物卡与关系网
- `03_worldbuilding/` 世界观规则、势力、地点
- `04_chapter_memory/` 章节摘要、滚动记忆、canon 事实、未决问题、时间线、人物状态、Arc 压缩记忆、势力记忆、卷记忆
